...
원본서버 트래픽 통계를 제공한다. 
트래픽은 프로토콜별로 구분된다. (현재는 HTTP만 지원한다.)
...
.10.3. ``[vhostMin]``      http.allSessionAverage              Integer    원본서버 평균 HTTP세션 수
.10.4. ``[vhostMin]``      http.activeSessionAverage           Integer    원본서버 평균 HTTP세션 중 전송 중인 평균 세션수
.10.10. ``[vhostMin]``     http.reqHeaderSize                  Integer    원본서버로 보내는 평균 HTTP Header 트래픽(Bytes)
.10.11. ``[vhostMin]``     http.reqBodySize                    Integer    원본서버로 보내는 평균 HTTP Body 트래픽(Bytes)
.10.12. ``[vhostMin]``     http.resHeaderSize                  Integer    원본서버로부터 받는 평균 HTTP Header트래픽(Bytes)
.10.13. ``[vhostMin]``     http.resBodySize                    Integer    원본서버로부터 받는 평균 HTTP Body트래픽(Bytes)
.10.14. ``[vhostMin]``     http.reqAverage                     Integer    원본서버로 보낸 평균 HTTP요청 개수
.10.15. ``[vhostMin]``     http.reqCount                       Integer    원본서버로 보낸 HTTP요청 개수
...
.10.23. ``[vhostMin]``     http.res2xxTimeComplete             Integer    원본서버로부터 2xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.33. ``[vhostMin]``     http.res3xxTimeComplete             Integer    원본서버로부터 3xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.43. ``[vhostMin]``     http.res4xxTimeComplete             Integer    원본서버로부터 4xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.53. ``[vhostMin]``     http.res5xxTimeComplete             Integer    원본서버로부터 5xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.80. ``[vhostMin]``     http.resTotalAverage                Integer    원본서버가 보낸 전체 평균 HTTP응답 개수
.10.81. ``[vhostMin]``     http.resTotalCompleteAverage        Integer    원본서버로부터 성공한 평균 HTTP트랜잭션 개수
.10.82. ``[vhostMin]``     http.resTotalTimeRes                Integer    원본서버로부터 응답 헤더를 받을때까지 평균 소요시간(0.01ms)
.10.83. ``[vhostMin]``     http.resTotalTimeComplete           Integer    원본서버로부터 응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
.10.84. ``[vhostMin]``     http.resTotalCount                  Integer    원본서버가 보낸 전체 HTTP응답 개수
.10.85. ``[vhostMin]``     http.resTotalCompleteCount          Integer    원본서버로부터 성공한 HTTP트랜잭션 개수
.10.90. ``[vhostMin]``     http.connectTimeoutAverage          Integer    평균 원본서버 접속실패 횟수
.10.91. ``[vhostMin]``     http.receiveTimeoutAverage          Integer    평균 원본서버 전송실패 횟수
.10.92. ``[vhostMin]``     http.connectAverage                 Integer    평균 원본서버 접속성공 횟수
.10.93. ``[vhostMin]``     http.dnsQueryTime                   Integer    원본서버 접속 시 평균 DNS쿼리 소요시간
.10.94. ``[vhostMin]``     http.connectTime                    Integer    원본서버 평균 접속 소요시간(0.01ms)
.10.95. ``[vhostMin]``     http.connectTimeoutCount            Integer    원본서버 접속실패 횟수
.10.96. ``[vhostMin]``     http.receiveTimeoutCount            Integer    원본서버 전송실패 횟수
.10.97. ``[vhostMin]``     http.connectCount                   Integer    원본서버 접속성공 횟수
.10.98. ``[vhostMin]``     http.closeAverage                   Integer    전송 중 원본서버에서 먼저 소켓을 종료한 평균 횟수
.10.99. ``[vhostMin]``     http.closeCount                     Integer    전송 중 원본서버에서 먼저 소켓을 종료한 횟수
========================== =================================== ========== ===================================================================
...
.40. ``[vhostMin]``        NetStream.OnPlayStatus                     Integer    NetStream으로 OnPlayStatus 콜백을 보낸 횟수
.41. ``[vhostMin]``        NetStream.OnStatus                         Integer    NetStream으로 OnStatus 콜백을 보낸 횟수
========================== ========================================== ========== =============================================================
...
.10.13. ``[vhostMin]`` . ``[vhostIndex]``     http.resTotalTimeComplete             Integer    원본서버로부터 응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.23. ``[vhostMin]`` . ``[vhostIndex]``     http.res2xxTimeComplete               Integer    원본서버로부터 2xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.33. ``[vhostMin]`` . ``[vhostIndex]``     http.res3xxTimeComplete               Integer    원본서버로부터 3xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.43. ``[vhostMin]`` . ``[vhostIndex]``     http.res4xxTimeComplete               Integer    원본서버로부터 4xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.53. ``[vhostMin]`` . ``[vhostIndex]``     http.res5xxTimeComplete               Integer    원본서버로부터 5xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.13. ``[vhostMin]`` . ``[vhostIndex]``     http.resTotalTimeComplete                 Integer    클라이언트 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.23. ``[vhostMin]`` . ``[vhostIndex]``     http.res2xxTimeComplete                   Integer    클라이언트 2xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.33. ``[vhostMin]`` . ``[vhostIndex]``     http.res3xxTimeComplete                   Integer    클라이언트 3xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.43. ``[vhostMin]`` . ``[vhostIndex]``     http.res4xxTimeComplete                   Integer    클라이언트 4xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)
...
.10.53. ``[vhostMin]`` . ``[vhostIndex]``     http.res5xxTimeComplete                   Integer    클라이언트 5xx응답 HTTP 트랜잭션 평균 완료시간(0.01ms)

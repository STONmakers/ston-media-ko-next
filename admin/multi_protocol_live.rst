...
이기종간의 프로토콜 변환이 발생할 때(RTMP to HLS/HLS to RTMP) 어떤 제약이 있는지 반드시 이해해야 한다.
...
.. note::
   ...
   채널은 메모리만을 이용해 동작하기 때문에 메모리 크기따라 생성할 수 있는 채널 수는 제한된다. 
   예를 들어 사용가능한 메모리가 10GB이고 채널 하나가 10MB를 소비한다면 약 1000개의 채널이 서비스 가능하다.
...
(HLS처럼 연결을 유지하지 않는 프로토콜의 경우) 채널을 즉시 파괴하면 클라이언트가 다시 접속했을 때 서비스 반응성이 떨어질 수 있다. 
따라서 서비스 특성에 맞추어 일정시간 채널을 유지하면 서비스 품질을 보장할 수 있다.

...
이 장에서는 각 프로토콜 클라이언트 연결과 요청을 처리하는 방식에 대해 설명한다.
프로토콜마다 설계철학과 동작방식이 상이하므로 특성을 잘 이해할 필요가 있다.
...
-  ``<ClientKeepAliveSec> (기본: 10초)``
   아무런 통신이 없는 상태로 설정된 시간이 경과하면 RTMP 클라이언트에게 Ping Request을 보낸다.
...
HTTP 클라이언트는 하나의 연결을 통해 여러 번의 요청/응답을 진행한다.
...
STON 미디어 서버의 HTTP 연결 유지정책은 Apache의 정책을 따른다.
HTTP 헤더 값에 따른 변수가 많아 다소 복잡하다.
HTTP 연결 유지정책에 영향을 주는 요소는 다음과 같다.
